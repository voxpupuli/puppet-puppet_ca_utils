plan manage_ca_file::jpmc_sync_cas2 (
  TargetSpec $ca_host,
) {
  ...
  run_plan('manage_ca_file::sync_cas2', targets => $target, ca_hosts => $all_cas, crl_bundle => 'api', restart_puppetserver => true) #, task_run_host => $target)
